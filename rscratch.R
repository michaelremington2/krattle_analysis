...
#setwd("/home/mremington/Documents/krattle_analysis/krattle_analysis/test")
...
owl_exp1_parameters <- read.csv("parameters.csv",header=TRUE)
...
ggplot(org_info,aes(fill=data_type,x=exp_figure_label,y=avg_bp))+
  ...
  ggtitle('Discrete Bush Preference by Organism')+
...
exp1_per_cycle <- read.csv("per_cycle.csv",header=TRUE)
...
exp1_per_cycle_1_sim <- exp1_per_cycle %>% filter(sim_number==1,experiment=='control', cycle>=90000) %>% group_by(org,cycle) %>% summarise(avg_bp = max(bush_pw_mean),
                                                                                                       sd_bush_pref = max(bush_pw_std))
exp2_per_cycle_1_sim <- exp1_per_cycle %>% filter(sim_number==1,experiment=='experiment1', cycle>=90000)%>% group_by(org,cycle) %>% summarise(avg_bp = max(bush_pw_mean),
                                                                                                      sd_bush_pref = max(bush_pw_std))
exp3_per_cycle_1_sim <- exp3_per_cycle %>% filter(sim_number==6, cycle>=495000)%>% group_by(org,cycle) %>% summarise(avg_bp = max(bush_pw_mean),
                                                                                                      sd_bush_pref = max(bush_pw_std))
exp4_per_cycle_1_sim <- exp4_per_cycle %>% filter(sim_number==0, cycle>=495000)%>% group_by(org,cycle) %>% summarise(avg_bp = max(bush_pw_mean),
                                                                                                      sd_bush_pref = max(bush_pw_std))
exp5_per_cycle_1_sim <- exp5_per_cycle %>% filter(sim_number==0, cycle>=495000)%>% group_by(org,cycle) %>% summarise(avg_bp = max(bush_pw_mean),
                                                                                                      sd_bush_pref = max(bush_pw_std))
exp6_per_cycle_1_sim <- exp6_per_cycle %>% filter(sim_number==0, cycle>=495000)%>% group_by(org,cycle) %>% summarise(avg_bp = max(bush_pw_mean),
                                                                                                      sd_bush_pref = max(bush_pw_std))
...
pc3<-ggplot(exp3_per_cycle_1_sim, aes(x=cycle, y=avg_bp, group=org, color=org))+
  ...
  ylab("Bush Preference")+
  xlab("Cycle")+
  ggtitle('Exp 3: Krats 360/Snakes 80/Owls 40')+
  theme(plot.title = element_text(size = 14, face = "bold"))+
  theme(text=element_text(size=12, face = "bold"))+
  ylim(0,1)+
  scale_color_manual("Organism", values = c("krat" = "#FFC20A", "snake" = "#0C7BDC"))
...
pc4<-ggplot(exp4_per_cycle_1_sim, aes(x=cycle, y=avg_bp, group=org, color=org))+
  ...
  ylab("")+
  xlab("")+
  ggtitle('Exp 4: Krats 360/Snakes 60/Owls 60')+
  theme(plot.title = element_text(size = 14, face = "bold"))+
  theme(text=element_text(size=12, face = "bold"))+
  ylim(0,1)+
  scale_color_manual("Organism", values = c("krat" = "#FFC20A", "snake" = "#0C7BDC"))
...
pc5<-ggplot(exp5_per_cycle_1_sim, aes(x=cycle, y=avg_bp, group=org, color=org))+
  ...
  ylab("")+
  xlab("")+
  ggtitle('Exp 5: Krats 360/Snakes 40/Owls 80')+
  theme(plot.title = element_text(size = 14, face = "bold"))+
  theme(text=element_text(size=12, face = "bold"))+
  ylim(0,1)+
  scale_color_manual("Organism", values = c("krat" = "#FFC20A", "snake" = "#0C7BDC"))
...
pc6<-ggplot(exp6_per_cycle_1_sim, aes(x=cycle, y=avg_bp, group=org, color=org))+
  ...
  ylab("")+
  xlab("Cycle")+
  ggtitle('Exp 6: Krats 360/Snakes 20/Owls 100')+
  theme(plot.title = element_text(size = 14, face = "bold"))+
  theme(text=element_text(size=12, face = "bold"))+
  ylim(0,1)+
  scale_color_manual("Organism", values = c("krat" = "#FFC20A", "snake" = "#0C7BDC"))
...
ggsave('/home/mremington/Documents/krattle_analysis/krattle_analysis/test/phenotype_model.png')
